...
{ 
	...
	## For windows server 2012+ machine
	Invoke-Sqlcmd -ServerInstance $SQLServer -Database $SQLDBName -Username $SQLUsername -Password $SQLPassword -InputFile $sqlScript | Export-Csv $OutputCSV -NoTypeInformation
}
